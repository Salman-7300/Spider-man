...
async function starte(breite, hoehe, seed, opt) {
  ...
  await page.addInitScript((ein) => {
    ...
    /* Nur zum Messen: die Dachkrone im Stand vor problem-1 Punkt 6. */
    ...
    /* Nur zum Messen: Eckenwechsel ohne Bogen, Stand vor problem-2 A. */
    if (ein.eckAlt) window.__WEBHERO_ECK_ALT = 1;
    ...
    /* Nur zum Messen: Kamera zieht immer gleich langsam nach. */
    if (ein.kamNachAlt) window.__WEBHERO_KAMNACH_ALT = 1;
    ...
    /* Nur zum Messen: CLIMB_TOP_OUT beginnt wie vorher fest 1,2 m unter
       der Kante (ohne Vorausschau auf den Kamerapfad). */
    if (ein.topVorausAlt) window.__WEBHERO_TOPVORAUS_ALT = 1;
    ...
    /* Nur zum Messen: Rumpfpruefung aus (Stand vor Ort-1-Fix). */
    if (ein.rumpfAlt) window.__WEBHERO_RUMPF_ALT = 1;
    ...
    if (ein.nahtAlt) window.__WEBHERO_NAHT_ALT = 1;
       ...
       topVorausAlt: !!(opt && opt.topVorausAlt),
